{
  let mainContainer,
    ...
    cameraTarget = new THREE.Vector3(0, 0, 700),
    windowWidth,
    windowHeight,
    windowHalfWidth,
    windowHalfHeight,
    ...
    stats,
    contentElement,
    colors = ["#E9C46A", "#2A9D8F", "#264653", "#2A9D8F", "#264653"],
    graphics,
    currentGraphic = 0,
    graphicCanvas,
    gctx,
  ...
  Particle.prototype.init = function (i) {
    ...
    for (var i = 0; i < box.geometry.vertices.length; i++) {
      box.geometry.vertices[i].x += -2 + Math.random() * 5;
      box.geometry.vertices[i].y += -2 + Math.random() * 5;
      box.geometry.vertices[i].z += -2 + Math.random() * 5;
    }
  ...
  const createBgObject = (i) => {
    ...
    const material = new THREE.MeshBasicMaterial({ color: 0xc2ced2 });
